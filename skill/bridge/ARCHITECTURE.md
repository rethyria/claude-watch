...
| `permissions.js` | Pending permission maps and the blocking `waitForPermission()` / `resolvePermission()` / `cancelPermission()` trio. The auto-deny timeout fires a safety margin *before* the hook's client-side timeout (`HOOK_PERMISSION_TIMEOUT_MS`, shared with `setup-hooks.sh`'s `PERMISSION_HOOK_TIMEOUT_S`) so expiry never races; `cancelPermission()` cleans up when the blocking hook request is aborted; a registered SSE sync provider re-sends every pending prompt to each connecting client so ring-buffer eviction can never lose one. |
...
| `hooks.js` | HTTP handlers for the `/hooks/*` surface (permission, tool-output, stop, task-complete, error). The blocking permission handler answers an immediate no-decision when zero SSE clients are connected (the terminal dialog appears normally instead of stalling), and registers a close handler that cancels the pending entry and emits `permission-cleared` if Claude Code aborts the request. |
