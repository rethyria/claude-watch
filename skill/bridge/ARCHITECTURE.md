...
- **Auth contract:** until the surfaces deliberately diverge, tokens issued by either `/pair` or `/v1/pair` are valid on both surfaces. When per-device tokens land on `/v1` (issue #7), legacy-issued tokens must keep working on both surfaces for existing clients.
