...
test("permission expiry returns no decision and announces itself", async () => {
  ...
  const { waitForPermission, pendingPermissions } = await import("../permissions.js");
  const { sseBuffer } = await import("../transport-sse.js");

  const decision = await waitForPermission("perm-timeout-1");
  // NOT a deny: a fabricated deny reaches the agent as a decision the user
  // never made (issue #63).
