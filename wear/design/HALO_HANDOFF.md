...
| Spawn as trailing "+ new session" pager card | The prototype has no spawn affordance; the card is EVERY scope's true end (› hidden there) and the empty-All content; ring highlight fades while it's selected. Same `haloSpawn` tag/picker as v1. _The epic's All-only slot was SUPERSEDED by field use (#130, user ask 2026-08-16): project pagers append the same slot. The first shape (picker with the project preselected) was itself SUPERSEDED the same day by the user's verdict — "instantly spawn for that project, not provide a menu" — so a project-scoped card spawns DIRECTLY into the project's spawn root (repoRoot-beats-cwd); navigating to the true end and tapping IS the deliberate act. The picker now opens only from All (the project is a genuine question there) or the vanished-project fallback._ |
