...
| Spawn as trailing "+ new session" pager card | The prototype has no spawn affordance; the card is EVERY scope's true end (› hidden there) and the empty-All content; ring highlight fades while it's selected. Same `haloSpawn` tag/picker as v1. _The epic's All-only slot was SUPERSEDED by field use (#130, user ask 2026-08-16): project pagers append the same slot, and a project-scoped card opens the picker with its own project PRESELECTED — hoisted first, marked, one confirm tap, the request carrying that project's spawn root. Preselect over a direct no-picker spawn (the recorded implementer's call): the picker still offers real choices with the project fixed — every other target, home, cancel — so the All flow's deliberate-action guard and muscle memory survive the scoping._ |
